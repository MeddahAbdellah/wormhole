...
export class LoginPage implements OnInit {
  ...
  connect(email,password){
   ...
   else if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,30}$/.test(password)))this.presentToast('Password must contain from 8 to 30 characters, 1 uppercase, 1 lowercase, 1 number, 1 special character. ')
   else{
     let hashedPwd = new jsSHA("SHA-256","TEXT");
     hashedPwd.update(password);
     this.http.post('http://ec2-52-47-201-30.eu-west-3.compute.amazonaws.com/login',
      {
        email: email,
      ...
      .then(data => {
        ...
        this.presentToast("Connexion Successful!");
